...
당뇨·고혈압·신장질환을 관리하며 제주 여행을 계획하는 5060 시니어에게는 일반 여행앱이 놓치는 걱정거리가 있습니다.

- 향토음식점에 가도 이 음식이 내 혈당·혈압·신장에 안전한지 알 방법이 없다
- 낯선 지역에서 응급상황이 생기면 어느 병원으로 가야 할지 모른다
- 무더위·미세먼지가 심한 날에도, 오래 걸어야 하는 코스밖에 추천받지 못한다
- 거동이 불편해도 안심하고 다닐 수 있는 여행지인지 미리 확인할 수 없다

## 해결 방법

"꽃보다 제주"는 건강 프로필(당뇨/고혈압/신장질환)을 한 번 설정하면 이후 모든 화면이 그 조건에 맞춰 개인화되는 AI 기반 제주 웰니스 여행 앱입니다. 향토음식은 식약처 실측 데이터를 근거로 위험도를 점수화하고, 응급의료기관·대기질·자외선지수·무장애 여행지는 공공데이터를 실시간으로 반영합니다.

## 주요 기능

- **건강 프로필 설정**: 당뇨, 고혈압, 신장 질환 등 건강 요인/관심사를 선택
...
- **향토 음식 영양 분석**: 식약처 식품영양성분DB로 실측 영양치를 조회하고, Gemini AI가 그 데이터를 근거로 위험도 점수(당뇨/고혈압/신장)와 안전하게 먹는 요령·대체 음식을 제안 (근거 데이터가 없을 땐 AI 추정치로 자동 대체)
